...
def main():
    ...
    # Make the command optional so that running `jcat` by itself works.
    ...
    try:
        # The 'config' command is special since it doesn't need an API key.
        ...
        if args.command is None:
            handle_session_interactive(client, None)
        elif hasattr(args, 'func'):
            args.func(args, client)
        else:
            # This can happen if a command is specified without a subcommand,
            # e.g., 'jcat session'. In this case, show the help message.
